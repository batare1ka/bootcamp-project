...
(() => {
/*!**************************************************!*\
  !*** ./resources/js/products/product-details.js ***!
  \**************************************************/
...
_toConsumableArray(smallimg).map(function (e) {
  return e.onclick = function () {
    MainImg.src = e.src;
  };
});

var addToCartBtn = document.querySelector(".add__to-cart");
...
var randomProducts = document.querySelectorAll(".product");
randomProducts.forEach(function (rand) {
  rand.addEventListener("click", function (event) {
    var title = rand.querySelector(".p-name").textContent;
    var randProd = listRandProducts.find(function (prod) {
      return prod.name == title;
    });

    if (!event.target.classList.contains("buy-btn")) {
      location.href = "http://localhost:880/product/".concat(randProd.id);
    } else {
      if (window.getComputedStyle(cartCount, null).display === "none") {
        cartCount.style.display = "flex";
      }
