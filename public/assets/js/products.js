...
(() => {
/*!*******************************************!*\
  !*** ./resources/js/products/products.js ***!
  \*******************************************/
var _require = __webpack_require__(/*! axios */ "./node_modules/axios/index.js"),
    axios = _require["default"];

var productTemplate = document.querySelector("[product-template]");
var insertToPage = document.getElementById("product-data");
var preloader = document.querySelector('.ajax-load');
  ...
  axios.get(url, {
    ...
    if (data == "") {
      ...
      data.forEach(function (product) {
        var productOnPage = productTemplate.content.cloneNode(true).children[0];
        productOnPage.addEventListener("click", function (event) {
          return goToProductPageOrAddCart(event, product);
        });
        productOnPage.querySelector(".p-name").textContent = product.name;
        productOnPage.querySelector(".product-price").textContent = "$ ".concat(product.price.toFixed(2));
...
function goToProductPageOrAddCart(event, product) {
  if (event.target.classList.contains("store-item-icon") || event.target.classList.contains("fa-shopping-cart")) {
    ...
    window.location.href = "/product/".concat(product.id);
  }
}

var brandList = document.querySelector("[brand-list]");
var brandTemplate = document.querySelector("[brand-item]");

function uncheckInputs(leaveChecked, identifier) {
  var allChildInputs = document.querySelectorAll(identifier);
  allChildInputs.forEach(function (element) {
...
function removeAllChildNodes() {
  ...
  productsToRemove.forEach(function (element) {
    element.remove();
  });
}
...
function fetchBrands() {
  axios.get("/api/brands/").then(function (_ref2) {
    ...
    data.forEach(function (brand) {
      var brandOnPage = brandTemplate.content.cloneNode(true).children[0];
      brandOnPage.innerHTML += brand.name;
      var checkOrNot = brandOnPage.querySelector(".brand-input");
...
var categoryTemplate = document.querySelector("[category-item]");

function fetchCategories() {
  axios.get("/api/categories/").then(function (_ref3) {
    var data = _ref3.data;
    data.forEach(function (category) {
      var categoryOnPage = categoryTemplate.content.cloneNode(true).children[0];
      categoryOnPage.innerHTML += category.name;
      var checkOrNot = categoryOnPage.querySelector(".category-input");
...
fetchBrands();
fetchCategories();
var sortNewest = document.querySelector(".sort__newest");
var sortLatest = document.querySelector(".sort__latest");

sortNewest.onchange = function () {
...
var sortHigh = document.querySelector(".sort__high");
var sortLow = document.querySelector(".sort__low");

sortHigh.onchange = function () {
  if (sortHigh.checked) {
...
function addProductsToCart(product) {
  ...
  if (!cart) {
    ...
    var prevProducts = JSON.parse(cart);
    var productExistsInArray = prevProducts.some(function (element) {
      return element.id === product.id;
    });

    if (productExistsInArray) {
      prevProducts.forEach(function (element) {
